package com.datastrucutre.recursion;
...
public class MergeSorter {
	...
	private void merge(int[] data, int start, int mid, int end) {
		...
		for (int k = start; k <= end; k++) {
			...
			//if right is empty i.e j>=size of Right then popluate with left or 
			//if left is not empty and left is less than right then populate with left
